...
class ResetPassword(MDScreen):
    ...
    def on_pre_enter(self, *args):
        ...
        self.ids.text_redefined.text_color = (135/255,57/255,187/255,1)
        self.ids.text_redefined.text = "Envie um e-mail para redefinir a senha."
    ...
    def send_email_confirm(self):
        ...
        if accont.reset_password(email):
            ...
            self.ids.text_redefined.text = "Falha ao enviar e-mail"
            self.ids.text_redefined.theme_text_color = "Custom"
...
class Register(MDScreen):
    def on_pre_enter(self, *args):
        ...
        self.ids.label_register.text = "Crie seu Cadastro"
        self.ids.label_register.theme_text_color = "Custom"
        self.ids.label_register.text_color = (135/255,57/255,187/255,1)
    ...
    def validate_info(self):
        ...
        if password == password_confirm and "@" in email and ".com" in email and len(password) >= 6:
            if accont.sing_up(email, password):
                self.ids.label_register.text = "Cadastro criado com Sucesso"
                self.ids.label_register.theme_text_color = "Custom"
                self.ids.label_register.text_color = (0, 1, 0, 1)
                Clock.schedule_once(self.callback, 3)

            else:
                self.ids.label_register.text = "Falha ao cria o Cadastro"
                self.ids.label_register.theme_text_color = "Custom"
